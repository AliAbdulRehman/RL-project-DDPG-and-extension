...
class DDPGAgent(BaseAgent):
    ...
    def _update(self, i):
        ...
        # compute target q using target policy
        ...
        # compute critic loss
        critic_loss = torch.mean(torch.square(current_q - target_q))
        ...
        # compute actor loss
        action_recalculated = self.pi(batch.state)
        current_q_recalculated = self.q_target(batch.state, action_recalculated)
    ...
    def get_action(self, observation, evaluation=False):
        ...
        if evaluation:
            action = self.pi(x)
        elif self.buffer_ptr < self.random_transition: # collect random trajectories for better exploration.
            ...
            # Add noise
            gaussian = torch.randn((1, self.action_dim))
            action = (action + expl_noise * gaussian).clamp(-self.max_action, self.max_action)
